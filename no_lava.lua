...
local nothing_lava = minetest.registered_items["bucket:bucket_lava"].on_place

minetest.override_item("bucket:bucket_lava", {
	on_place = function(itemstack, placer, pointed_thing)
		if not minetest.check_player_privs(placer:get_player_name(),
				{place_lava = true}) then
			return itemstack
		else
			return nothing_lava(itemstack, placer, pointed_thing)
		end
	end,
})
...
minetest.override_item("bucket:bucket_empty", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		-- lava check
		if node.name == "default:lava_source" and not minetest.check_player_privs(user:get_player_name(), {place_lava = true}) then
			return
		end
		-- end lava check
